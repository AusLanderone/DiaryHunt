...
// ---------- what an open leg has accrued so far ----------
const openTrade = {
  usdRub: 84.2, payout: 0, adjustment: 0, closeDate: '', openDate: '2026-09-18',
  legs: [
...
const withInterim = (rub, extra = {}) => {
  const leg = { ...openTrade.legs[0], ...extra };
  return { ...openTrade, legs: [{ ...leg, vmOpenRub: rub,
    vmOpenMeta: { secid: 'GDZ6', sessions: 2, through: '2026-09-18', computedAt: '2026-09-21T10:00:00.000Z',
      fingerprint: calc.vmFingerprint(leg, openTrade) } }, openTrade.legs[1]] };
};

test('an open leg reports what has already been credited', () => {
  const t = withInterim(21420);
  near(calc.legInterimRub(t.legs[0], t), 21420);
  assert.strictEqual(calc.legInterimRub(openTrade.legs[0], openTrade), null, 'nothing computed, nothing to show');
  const c = calc.computeTrade(t);
  near(c.legs[0].interimRub, 21420);
  assert.strictEqual(c.legs[0].interimThrough, '2026-09-18');
  assert.strictEqual(c.legs[1].interimRub, null, 'the other venue has no such figure');
});

test('an accrual is not a result: the trade is still open and still has no PnL', () => {
  const t = withInterim(21420);
  assert.strictEqual(calc.isClosed(t), false);
  assert.strictEqual(calc.pnlRub(t), null);
  assert.strictEqual(calc.netProfitRub(t), null);
  assert.strictEqual(calc.legGrossRub(t.legs[0], t.usdRub, t), null);
});

test('an accrual computed for a different shape is dropped like any other', () => {
  const t = withInterim(21420);
  const moved = { ...t, legs: [{ ...t.legs[0], units: 20 }, t.legs[1]] };
  assert.strictEqual(calc.legInterimRub(moved.legs[0], moved), null);
});

test('once the leg is closed the accrual gives way to the real figure', () => {
  const t = withInterim(21420, { exitPrice: 4400 });
  assert.strictEqual(calc.legInterimRub(t.legs[0], t), null);
});
