...
// ---- unrealised result of an open trade ----

// an open SILV trade: entered, not closed, with current prices noted
const openMarked = {
  usdRub: 85, payout: 0, adjustment: 0, closeDate: '',
  legs: [
    { exchange: 'MOEX', side: 'Шорт', entryPrice: 69.28, units: 1060, exitPrice: null, feeRub: 100, markPrice: 68.9 },
    { exchange: 'FOREX', side: 'Лонг', entryPrice: 68.9955, units: 1000, exitPrice: null, feeRub: 0, markPrice: 68.7 },
  ],
};

test('unrealized — values an open trade at the noted prices', () => {
  const u = calc.unrealized(openMarked);
  assert.ok(u, 'an open trade with marks has an unrealised result');
  // short: 69.28 -> 68.90 on 1060 = +402.80 $; long: 68.9955 -> 68.70 on 1000 = -295.50 $
  near(u.pnlRub, (402.8 - 295.5) * 85 - 100, 0.5);
  near(u.netProfitRub, u.pnlRub, 0.001);       // no payout, swap or adjustment here
});

test('unrealized — the spread now, measured like any other spread', () => {
  const u = calc.unrealized(openMarked);
  near(u.exitSpread * 100, ((68.7 - 68.9) / ((68.7 + 68.9) / 2)) * 100, 0.0002);
  near(u.spreadTotal, calc.entrySpread(openMarked) - u.exitSpread, 1e-9);
});

test('unrealized — nothing to value without a mark on every leg', () => {
  assert.strictEqual(calc.unrealized({ ...openMarked,
    legs: [openMarked.legs[0], { ...openMarked.legs[1], markPrice: null }] }), null);
  assert.strictEqual(calc.unrealized({ ...openMarked,
    legs: openMarked.legs.map((l) => ({ ...l, markPrice: undefined })) }), null);
});

test('unrealized — a closed trade has a real result, not an unrealised one', () => {
  assert.strictEqual(calc.unrealized(trade1), null);
});

test('unrealized — payout, swap and adjustment are carried into the estimate', () => {
  const t = {
    ...openMarked, payout: 1500, adjustment: -200,
    legs: openMarked.legs.map((l, i) => (i === 0 ? { ...l, swap: -40 } : l)),
  };
  const u = calc.unrealized(t);
  near(u.netProfitRub, u.pnlRub + 1500 - 40 - 200, 0.5);
});
